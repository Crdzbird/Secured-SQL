...
create or replace function str_clean(p_text text, p_remove_multiple_blanks boolean, p_remove_linebreaks boolean, p_remove_tabs boolean, p_remove_multiple_backslashes boolean, p_remove_htmlentities boolean, p_html_unencode boolean, p_remove_html boolean) returns text
    language plpgsql
as
$$
...
BEGIN
    ...
    IF (p_remove_html = TRUE ) THEN
        IF (v_result ~* '<*>') THEN
            WHILE POSITION( '<' IN v_result ) > 0 AND POSITION('>' IN v_result ) > 0 LOOP
                    v_start := POSITION( '<' IN v_result );
                    v_end   := POSITION( '>' IN v_result );
                    v_len   := ( v_end - v_start) + 1;
                    IF (v_len > 0) THEN
                        v_result := REPLACE( v_result, SUBSTRING(v_result, v_start, v_len ), '');
                    END IF;
                END LOOP;
        END IF;
    END IF;
...
create or replace function str_html_unencode(x text) returns text
    language plpgsql
as
$$
DECLARE
